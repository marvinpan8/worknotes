## [如何保护对外暴露的 Kubernetes 服务](<https://yq.aliyun.com/articles/698191>)

[chuanhu.yang](https://yq.aliyun.com/users/6dzmqjq3iw67a) 2019-04-14 22:06:47 浏览310
...
首先，我们需要创建用于存储用户名和密码的`htpasswd`文件：`yum install httpd-tools`

```bash
# 1、 新建一个密码文件.passwd并添加一个用户,不提示直接输入用户名密码
$ htpasswd -bc auth test investoday
Adding password for user test
#2、 在原有的密码文件.passwd下在添加一个用户
$ htpasswd -b auth wen jiang
#3、更新用户的密码:有两种方式
#第一种,直接添加相同的用户名，就会自动去更新密码：
$ htpasswd -b auth wen wen
#第二种,先删除需要更新密码的用户名,在添加用户：
$ htpasswd -D auth wen & htpasswd -b auth wen wen
...
-------------------------------------
$ kubectl delete secret mysecret -n dataapi-prod
$ kubectl create secret generic mysecret --from-file=auth -n dataapi-prod
```
...
![jenkins basic auth](assets/006tNc79gy1g213ot1nyjj314y0qy422.jpg)

## OAuth 认证

除了上面的 Basic Auth 认证方式以为，我们还可以通过 Github、Google 等提供的 OAuth 服务来进行身份验证。我们可以通过名为[OAuth2 Proxy](https://yq.aliyun.com/go/articleRenderRedirect?url=https%3A%2F%2Fgithub.com%2Fpusher%2Foauth2_proxy)的工具来代理请求，它通过提供一个外部身份验证的反向代理来实现，使用起来也相对简单。

### 安装

首先我们需要为我们的应用添加自动的 HTTPS，可以参考我们前面的文章[使用 Let's Encrypt 实现 Kubernetes Ingress 自动化 HTTPS](https://yq.aliyun.com/go/articleRenderRedirect?url=https%3A%2F%2Fwww.qikqiak.com%2Fpost%2Fautomatic-kubernetes-ingress-https-with-lets-encrypt%2F)。

然后登录 Github，在[https://github.com/settings/applications/new](https://yq.aliyun.com/go/articleRenderRedirect?url=https%3A%2F%2Fgithub.com%2Fsettings%2Fapplications%2Fnew)添加一个新的`OAuth`应用程序：

![register github app](assets/006tNc79gy1g21yei4dgej31600nmq64.jpg)
...
> 对于 GitHub，我们可以通过`github-org`和`github-team`来限制访问，一般设置`email-doamin="*"`，我们可以通过`OAuth2 Proxy`的[示例文档](https://yq.aliyun.com/go/articleRenderRedirect?url=https%3A%2F%2Fgithub.com%2Fpusher%2Foauth2_proxy%23github-auth-provider)来查看更改 GitHub Provider 的配置。
...
![github oauth](assets/006tNc79gy1g21yeyk5m5j312x0u043z.jpg)

然后认证通过后就可以跳转到我们的 Jenkins 服务了：

![jenkins](assets/006tNc79gy1g21yhlshzjj318x0u0tgq.jpg)

当然除了使用 GitHub 之外，还可以使用其他的 OAuth 认证服务，比如 Google，我们可以根据需要自行去添加即可。

## 相关链接

- [OAuth2 Proxy](https://yq.aliyun.com/go/articleRenderRedirect?url=https%3A%2F%2Fgithub.com%2Fpusher%2Foauth2_proxy)
- [ingress-nginx oauth external auth exmaple](https://yq.aliyun.com/go/articleRenderRedirect?url=https%3A%2F%2Fgithub.com%2Fkubernetes%2Fingress-nginx%2Ftree%2Fmaster%2Fdocs%2Fexamples%2Fauth%2Foauth-external-auth)
- [Kubernetes Helm 初体验](https://yq.aliyun.com/go/articleRenderRedirect?url=https%3A%2F%2Fwww.qikqiak.com%2Fpost%2Ffirst-use-helm-on-kubernetes%2F)
- [使用 Let's Encrypt 实现 Kubernetes Ingress 自动化 HTTPS](https://yq.aliyun.com/go/articleRenderRedirect?url=https%3A%2F%2Fwww.qikqiak.com%2Fautomatic-kubernetes-ingress-https-with-lets-encrypt%2F)