...
```bash
sudo git clone https://github.com/ahmetb/kubectx /jrtz/kubectx
sudo ln -s /jrtz/kubectx/kubectx /usr/local/bin/kubectx
sudo ln -s /jrtz/kubectx/kubens /usr/local/bin/kubens
```
## 安装kube-ps1

```bash
git clone https://github.com/marvinpan8/kube-ps1.git /jrtz/kube-ps1
```
...
Kube-prompt 使用 Go 语言开发，天生良好的跨平台性。安装起来非常简单，只需下载各平台对应的二进制版本就可以开箱即用。
