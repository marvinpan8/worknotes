...
- **/var/lib/glusterd：** 配置文件，**很重要**
  ...
  - vols/vol_xxx/vol_xxx.192.168.10.122.var-lib-heketi-mounts-vg_xxx-brick_xxx-brick.vol  此有vol下的brick配置文件，有几个brick,就有几个此文件
  - brick/192.168.10.122_-var-lib-heketi-mounts-vg_xxxxxx-brick_xxxxx-brick  此文件为brick进程启动配置文件
...
> 注：如果状态Online项为“N”的GH01存在PID号（不显示N/A）应当使用如下命令结束掉进程方可继续下面步骤。`kill -15 pid`
>
...
- **硬链接(推荐)：**
  ...
  有的系统 vg_099dabdc48c0ce612fb3a4e29719398b-brick_19c7e5e21e6cc01ff3c645055f244833 直接就是块设备（b开头的文件）

```bash
ln -d ../dm-39 vg_fbce95ef571b0e6e34d8a888feb3758c-brick_33ad154221d44c66322f801215d42f51
```

fdisk -l    从前面对比，可以发现**3个块设备**未 `mount`

```bash
[root@vmsrv-010-143 /]# fdisk -l
......
...
使用 `h topology info` 查看，对应的具体详细是哪个 pvc
...
## 3. gluster peer hostname修复

使用`gluster peer status` 查看状态，Hostname值不是IP，需要重新定义为IP，Other names为真正的hostname: vmsrv-010-122
