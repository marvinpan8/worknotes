...
by Michael Fischer | on 12 MAR 2019 | in [Open Source](https://aws.amazon.com/cn/blogs/china/category/open-source/) | [Permalink](https://aws.amazon.com/cn/blogs/china/demystifying-entrypoint-cmd-docker/) | [ Share](https://aws.amazon.com/cn/blogs/china/demystifying-entrypoint-cmd-docker/?nc1=b_rp#)
...
1. 必须都可在运行时上单独覆盖；
2. 任何一个或两者都可为空；并且
3. 其中的加号 (+) 是指 `ENTRYPOINT` 与 `CMD` **在阵列环境中分别***串联*。
...
请注意 `CMD` 将被*完全替换* — 不能在其开头或结尾处添加任何字符。
...
假设我们为某个项目构建自己的 `Dockerfile`。我们已经了解了 `ENTRYPOINT` 和 `CMD` 如何结合起来构建容器的默认参数列表的*原理*。但现在我们需要知道*如何选择*：何时建议使用 `ENTRYPOINT`，何时又建议使用 `CMD`？
